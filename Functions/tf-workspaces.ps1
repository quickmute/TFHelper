...
<#
.Description
Return ALL workspaces with optional filtering. 
Example uses
# Get all baseline workspaces
$baselineWorkspaces = tf-workspaces-get-list -nameFilter "baseline-*"
# Get all production workspaces
$prodWorkspaces = tf-workspaces-get-list -nameFilter "*-prod"
# Get workspaces starting with "orc"
$orcWorkspaces = tf-workspaces-get-list -nameFilter "*orc*"
# Get baseline that has policy_checked
$filteredWorkspaces = tf-workspaces-get-list -nameFilter "baseline-*" -statusFilter "policy_checked"
# Get production baseline that is already applied
$filteredWorkspaces = tf-workspaces-get-list -nameFilter "baseline-*" -tagFilter @("production") -statusFilter "applied"
.PARAMETER nameFilter
Wildcard filter for workspace names (supports * wildcards, e.g., "baseline-*", "*-prod")
.PARAMETER tagFilter
Filter by tag names (array of strings, comma-separated in API call)
.PARAMETER statusFilter
Filter by current run status (e.g., "applied", "discarded", "planned_and_finished", "errored", "policy_checked", "policy_override")
#>
    param(
        $this_domain,
        $this_token,
        ...
        [string]$nameFilter,
        [string[]]$tagFilter,
        [ValidateSet("applied", "discarded", "planned_and_finished", "errored", "policy_checked", "policy_override")]
        [string]$statusFilter
    )
    ...
    $queryParams = @()
    $queryParams += "page%5Bnumber%5D=$pageNum"
    $queryParams += "page%5Bsize%5D=$pageSize"
    
    if ($nameFilter) {
        $queryParams += "search%5Bwildcard-name%5D=$nameFilter"
    }
    
    if ($tagFilter -and $tagFilter.Count -gt 0) {
        $tagString = $tagFilter -join ","
        $queryParams += "search%5Btags%5D=$tagString"
    }
    
    if ($statusFilter) {
        $queryParams += "filter%5Bcurrent-run%5D%5Bstatus%5D=$statusFilter"
    }
    
    do{ 
        # Update page number in query params
        $queryParams[0] = "page%5Bnumber%5D=$pageNum"
        $queryString = $queryParams -join "&"
        $endpoint = "/organizations/:orgName/workspaces?$queryString"
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $plan_id = tf-run-get-plan-id -run_id $run_id -this_domain $this_domain -this_token $this_token -this_org $this_org
        $plan_url = tf-run-get-plan-url -plan_id $plan_id -this_domain $this_domain -this_token $this_token -this_org $this_org
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        if ($current_status -notin $ignore_status_list){
            ...
            $response = tf-custom-invoke -endpoint $endpoint -Method Patch -Body $json_body -this_domain $this_domain -this_token $this_token -this_org $this_org
...
#>
    ...
    $response = tf-workspace-get-current-state -workspaceName $workspaceName -workspaceID $workspaceID  -this_domain $this_domain -this_token $this_token -this_org $this_org
    ...
    foreach ($item in $response.data.relationships.outputs.data){
        ...
        $outputInfo = tf-custom-invoke -endpoint $getOutputInfo -method Get -this_domain $this_domain -this_token $this_token -this_org $this_org
...
function tf-workspace-get-current-state{
    ...
    if ($workspaceName.Length -gt 0 -and $workspaceID.Length -eq 0){
        $workspaceID = tf-workspace-get-id -workspaceName $workspaceName  -this_domain $this_domain -this_token $this_token -this_org $this_org
    }
    ...
    if ($workspaceID.Length -gt 10){
        ...
        $state_output = tf-custom-invoke -endpoint $endpoint -method Get -this_domain $this_domain -this_token $this_token -this_org $this_org
    }
    return $state_output
}
...
function tf-workspace-get-current-run{
    ...
    $workspace = tf-workspaces-get-workspace -name $workspaceName -id $workspaceID -this_domain $this_domain -this_token $this_token -this_org $this_org
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $current_workspace = tf-workspaces-get-workspace -id $workspace_id -this_domain $this_domain -this_token $this_token -this_org $this_org
        ## get current run
        ...
        if ($current_status -in ('policy_checked', 'planned')){
            $response = tf-run-do-apply -run_id $run_id -comment $comment -this_domain $this_domain -this_token $this_token -this_org $this_org
        }else{  
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $current_workspace = tf-workspaces-get-workspace -id $workspace_id -this_domain $this_domain -this_token $this_token -this_org $this_org
        $run_id = $current_workspace.relationships.'current-run'.data.id      
        $response = tf-run-do-discard -run_id $run_id -comment $comment -this_domain $this_domain -this_token $this_token -this_org $this_org
    }
}
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $current_workspace = tf-workspaces-get-workspace -id $workspace_id -this_domain $this_domain -this_token $this_token -this_org $this_org
        $run_id = $current_workspace.relationships.'current-run'.data.id      
        $response = tf-run-do-cancel -run_id $run_id -comment $comment -this_domain $this_domain -this_token $this_token -this_org $this_org
    }
}
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $current_workspace = tf-workspaces-get-workspace -id $workspace_id -this_domain $this_domain -this_token $this_token -this_org $this_org
        $run_id = $current_workspace.relationships.'current-run'.data.id      
        $status = tf-run-get-status -run_id $run_id -this_domain $this_domain -this_token $this_token -this_org $this_org
...
If you want to force auto-apply then you can pass in argument flag 'auto_apply' otherwise it will
Use however that is configured PER workspace. Use with caution. 
#>
    param(
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSObject]
    ...
    foreach ($workspace in $workspaces){
        ...
        if ($auto_apply){
            $body.data.attributes."auto-apply" = $true
            $body.data.attributes."message" = $comment + " (Auto-Apply)"
        }
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        if ($current_status -in ('policy_override')){
            $policy_response = tf-run-get-policy-check -run_id $run_id -this_domain $this_domain -this_token $this_token -this_org $this_org
            ...
            if ($policy_response.data.attributes.permissions.'can-override'){
                $response = tf-run-override-soft-policy -policy_check_id $policy_response.data.id -this_domain $this_domain -this_token $this_token -this_org $this_org
            }else{  
...
#>
    ...
    $search = $prefix + "*"
    return $workspaces | Where-Object{$_.attributes.name -like $search}
}
    
function tf-workspaces-filter-by-name{
<#
.Description
Filter by given name, wildcard is accepted
#>
    param(
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSObject]
    ...
    if ($not){
        $output = $workspaces | Where-Object{$_.attributes.name -notlike $name}
    }else{
        $output = $workspaces | Where-Object{$_.attributes.name -like $name}
    }
    return $output
}


function tf-workspace-get-resources{
    <#
    .Description
    Get list of resources for a single workspace
    ...
    if ($workspaceID.Length -gt 10){
        do {
            $endpoint = "/workspaces/$workspaceID/resources?page%5Bnumber%5D=$pageNum&page%5Bsize%5D=$pageSize"
            $response = tf-custom-invoke -endpoint $endpoint -Method Get -this_domain $this_domain -this_token $this_token -this_org $this_org
...
#>
    ...
    foreach ($workspace in $workspaces){
        ...
        $workspace_id = $workspace.id
        $workspace_name = $workspace.attributes.name
        $output = tf-workspace-get-current-state -workspaceID $workspace_id -this_domain $this_domain -this_token $this_token -this_org $this_org
